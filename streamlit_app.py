...
def limpar_texto_pdf(txt):
    """Garante compatibilidade total com o Português Brasileiro no PDF."""
...
def create_pdf(p_name, hist, metrics, imgs):
    ...
    pdf.cell(0, 10, limpar_texto_pdf("PARECER TÉCNICO DE REABILITAÇÃO E EVOLUÇÃO"), ln=True, align='C')
    pdf.ln(5)
    ...
    # 2. SCORE IKDC - CENTRALIZAÇÃO E DESTAQUE (CORRIGIDO)
    ...
    # Moldura Centralizada do Score
    pdf.ln(2)
    pdf.set_fill_color(0, 128, 145) # Azul GENUA
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("helvetica", 'B', 14)
    # Cálculo para centralizar a caixa de 60mm
    pdf.set_x((pdf.w - 60) / 2)
    pdf.cell(60, 12, limpar_texto_pdf(f"RESULTADO: {metrics['ikdc']}/100 {metrics['ikdc_emoji']}"), ln=True, fill=True, align='C')
...
# --- 4. PAINEL ANALÍTICO (CÉREBRO TOTAL) ---
else:
    st.header("📊 Clinical Analytics & Decision Support")
    df = conn.read(ttl=0).dropna(how="all")
    if not df.empty:
        p_sel = st.selectbox("Selecione o Paciente", df['Paciente'].unique())
        df_p = df[df['Paciente'] == p_sel].copy()
        
        # Numeração de Sessões e Cruzamentos
        ...
        # --- GERAÇÃO DE GRÁFICOS (PORTUGUÊS REVISADO) ---
        
        # 1. Evolução Clínica (Dor 0-10, Função 0-10)
        ...
        ax_ev.set_ylim(-0.2, 10.5); ax_ev.set_ylabel("Escala (0-10)"); ax_ev.legend(); ax_ev.grid(True, alpha=0.1)
        # S10 em 10
        indices = np.arange(0, len(df_p), 10)
        ax_ev.set_xticks(indices); ax_ev.set_xticklabels([df_p['Sessao'].iloc[i] for i in indices])
        buf_ev = io.BytesIO(); plt.savefig(buf_ev, format='png', bbox_inches='tight'); plt.close(fig_ev)

        # 2. Histórico de Inchaço (0-3)
        fig_inc, ax_inc = plt.subplots(figsize=(10, 3))
        ax_inc.bar(df_p['Sessao'].tail(20), df_p['Inchaco_N'].tail(20), color='#008091', alpha=0.8, label='Inchaço')
        ax_inc.set_title("Linha do Tempo: Inchaço Articular (Stroke Test)", fontweight='bold')
        ax_inc.set_ylim(0, 3.2); ax_inc.set_ylabel("Grau (0-3)"); ax_inc.legend(); ax_inc.grid(axis='y', alpha=0.1)
        ...
        ax_cap.bar(testes, valores, color=['#008091', '#0097A7', '#00ACC1'], label='Nota do Teste')
        ax_cap.set_title("Perfil de Capacidade por Teste (Sessão Atual)", fontweight='bold')
        ax_cap.set_ylim(0, 10.5); ax_cap.set_ylabel("Nota (0-10)"); ax_cap.legend(); ax_cap.grid(axis='y', alpha=0.1)
        ...
        ax_s.set_title("Análise Biopsicossocial: Impacto do Sono na Dor", fontweight='bold'); ax_s.legend(); ax_s.set_ylim(0, 10.5)
        buf_s = io.BytesIO(); plt.savefig(buf_s, format='png', bbox_inches='tight'); plt.close(fig_s)

        # Dashboard Streamlit
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Dor Atual", f"{ultima['Dor']}/10", f"Status: {'Bom' if ultima['Dor']<=3 else 'Alerta'}")
        m2.metric("Inchaço", f"Grau {ultima[col_inc]}", f"Status: {'Estável' if ultima['Inchaco_N']<=1 else 'Irritado'}")
        m3.metric("IKDC Científico", f"{u_ikdc:.0f}/100", emoji_ikdc)
        m4.metric("Prognóstico Alta", prev_txt)

        st.write("---")
        t1, t2, t3 = st.tabs(["📈 Evolução & IA", "🌙 Biopsicossocial & Inchaço", "🎯 Capacidade & Postura"])
        with t1: st.pyplot(fig_ev); st.success(f"🔮 **Previsão de Alta:** Com base no ganho funcional, o paciente atingirá 90% em {prev_txt}.")
        with t2: st.pyplot(fig_inc); st.pyplot(fig_s)
        with t3: 
            st.pyplot(fig_cap)
            st.write("**Análise: Postura vs Prevalência de Dor**")
            st.altair_chart(alt.Chart(df_p).mark_bar(color='#008091').encode(x='Postura', y='mean(Dor)', tooltip=['Postura', 'mean(Dor)']), use_container_width=True)
        ...
        st.download_button("📥 BAIXAR RELATÓRIO CLÍNICO MASTER (PDF)", data=pdf_bytes, file_name=f"GENUA_Report_{p_sel}.pdf")
        
        # ZenFisio Text
        st.info(f"📝 ZenFisio: Paciente {p_sel} - Dor {ultima['Dor']}/10, Inchaço Grau {ultima[col_inc]}, IKDC {u_ikdc:.0f}/100. Alta prevista: {prev_txt}.")
    else: st.info("Planilha vazia. Aguardando dados.")
